...
app.post("/api/tx/getinfo", function(req, res) {
    ...
    query.getTransactionByID('peer1',ledgerMgr.getCurrChannel(),txid,'admin','org1').then(response_payloads=>{
        ...
        var wkeyi = data.actions[0].payload.action.proposal_response_payload.extension.results;
        var wkey;
        var wpayload;
        if (wkeyi.ns_rwset[0].namespace=="lscc"){
            wkey = wkeyi.ns_rwset[1].rwset.writes[0].key;
            wvalue = wkeyi.ns_rwset[1].rwset.writes[0].value;
        } else {
            wkey = wkeyi.ns_rwset[0].rwset.writes[0].key;
            wvalue = wkeyi.ns_rwset[0].rwset.writes[0].value;
        }
        var wpayload = data.actions[0].payload.action.proposal_response_payload.extension.response.payload;
